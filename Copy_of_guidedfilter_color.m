function q = Copy_of_guidedfilter_color(I, p, r, eps)
...
+ a(:, :, 2).* I(:, :, 2)...
+ a(:, :, 3).* I(:, :, 3)...
+ b) ;  % Eqn. (16) in the paper;


cesi=0.8;%树聚合时的权重
% a_ag(:, :, 1)= TreeTrans(a(:, :, 1),cesi,1);
% a_ag(:, :, 2)= TreeTrans(a(:, :, 2),cesi,1);
% a_ag(:, :, 3)= TreeTrans(a(:, :, 3),cesi,1);
% b_ag=TreeTrans(b,cesi,1);
% q = (a_ag(:, :, 1).* I(:, :, 1)...
% + a_ag(:, :, 2).* I(:, :, 2)...
% + a_ag(:, :, 3).* I(:, :, 3)...
% + b_ag) ;  % Eqn. (16) in the paper;
q=TreeTrans(q,cesi,5);
